...
AppdataPATH = os.path.join(
    dirs.user_data_dir, "OPENGOAL-UnofficalModLauncher", "")
...
installed_mods_column = [
    ...
    [
        ...
        sg.Btn(button_text="Add to Steam",
               key='AddToSteam', enable_events=True)
    ],
]
...
mod_list_column = [
    ...
    [sg.Text("Mod Creator")],
    [sg.Combo(list(listOfModsJSON.keys()), enable_events=True,
              key='pick_modder', size=(40, 0))],
    [sg.Text("Their Mods")],
...
def bootup():
    ...
    if subfolders == [] or subfolders[0] == "No Mods Installed":
        ...
        window['-SELECTEDMODIMAGE-'].update(
            githubUtils.resize_image(noimagefile, resize=(1, 1)))
        item = "Modding Community"
        window['pick_modder'].update(item)
        title_list = [i["name"] for i in listOfModsJSON[item]]
        ...
        currentModImage = None
        [currentModderSelected, currentModSelected,
            currentModURL, currentModImage] = handleModSelected()
    ...
    if subfolders != [] and subfolders[0] != "No Mods Installed":
        ...
        for modder in listOfModsJSON.keys():
            for mod in listOfModsJSON[modder]:
...
def handleModSelected():
    ...
    for mod in listOfModsJSON[tmpModderSelected]:
    ...
    try:
        ...
        if r == 200:
            ...
            window['-SELECTEDMODIMAGE-'].update(
                githubUtils.resize_image(png_data, resize=(250, 250)))
            # prints the int of the status code. Find more at httpstatusrappers.com :)
        else:
            window['-SELECTEDMODIMAGE-'].update(
                githubUtils.resize_image(noimagefile, resize=(250, 250)))
...
def open_steamPrompt():
    filelayout = [[sg.FolderBrowse(
        "Select Steam Directory", enable_events=True, key="PICKSTEAMDIR")]]
    filewindow = sg.Window('Search for offered mods', filelayout, keep_on_top=True,
                           icon=iconfile, size=window.size, location=window.CurrentLocation())
    ...
    while True:
        ...
        if exists(values[event]+"/steam.exe"):
            ...
            sg.popup("Did not find Steam install Directory try again",
                     keep_on_top=True)
...
def open_search():
    ...
    layout = [[sg.Text('Search for offered mods')],
              ...
              [sg.Listbox(names, size=window.size,
                          enable_events=True, key='-LIST-')],
              [sg.Button('Chrome'), sg.Button('Exit')]]

    window2 = sg.Window('Search for offered mods', layout, keep_on_top=True,
                        icon=iconfile, size=window.size, location=window.CurrentLocation())
# Event Loop
    while True:
        ...
        if values['-INPUT-'] != '':
            ...
            # do the filtering
            new_values = [x for x in names if search.lower() in x.lower()]
            ...
            if event == '-LIST-':
                ...
                window['pick_mod'].update(
                    value=title_list[indexOfMod], values=title_list)
                handleModSelected()
                window2.close()
...
while True:
    ...
    if event == "InstalledModListBox" and not window["InstalledModListBox"].get() == ['No Mods Installed']:
        ...
        if not tmpModderSelected:
            sg.Popup('Installed mod not found in available mods!',
                     keep_on_top=True, icon=iconfile)
            window['-SELECTEDMOD-'].update(tmpModSelected)
            ...
            if local_img:
                window['-SELECTEDMODIMAGE-'].update(
                    githubUtils.resize_image(local_img, resize=(250, 250)))
            else:
                window['-SELECTEDMODIMAGE-'].update(
                    githubUtils.resize_image(noimagefile, resize=(250, 250)))
        else:
            ...
            title_list = [i["name"]
                          for i in listOfModsJSON[tmpModderSelected]]
        ...
        if tmpModSelected and not tmpModSelected == "No Mods Installed" and tmpModSelected in subfolders:
            ...
            if (len(window['InstalledModListBox'].get())) == 0:
                bootup()
            sg.Popup('No installed mod selected',
                     keep_on_top=True, icon=iconfile)
        ...
        if tmpModSelected and not tmpModSelected == "No Mods Installed" and tmpModSelected in subfolders:
            ...
            ans = sg.popup_ok_cancel(
                'Confirm: uninstalling ' + dir, icon=iconfile)
            if ans == 'OK':
        ...
        if exists(steamDIR + "\steamapps\common\Play With Gilbert"):
            ...
            if sg.PopupYesNo('Do you want to replace Play With Gilbert with the unoffical Mod Launcher?') == "Yes":
                ...
                launcherUtils.try_remove_file(
                    steamDIR + "\steamapps\common\Play With Gilbert\PlayWithGilbert.exe")
                ...
                urllib.request.urlretrieve(
                    autoUpdaterURL, "PlaywithGilbert.exe", launcherUtils.show_progress)
                print("Done downloading")
                print("moving to steam")
                shutil.move("PlaywithGilbert.exe", steamDIR +
                            "\steamapps\common\Play With Gilbert\\")
                if sg.PopupYesNo('Do you ever want to play the real Play with Gilbert') == "Yes":
                    ...
                    sg.Popup(
                        "Ok removing the play with gilbert game files to save space. ( :( )")
                    launcherUtils.try_remove_dir(
                        steamDIR + "\steamapps\common\Play With Gilbert\Engine")
                    launcherUtils.try_remove_dir(
                        steamDIR + "\steamapps\common\Play With Gilbert\PWG_2020")
                sg.Popup(
                    "Mod Launcher added to steam, launch by playing \"Play with Gilbert\" in Steam.")
            else:
                sg.Popup("Understandable")

        else:
            sg.Popup("Did not find play with gilbert please download from " +
                     "https://store.steampowered.com/app/1359630/Play_With_Gilbert__A_Small_Tail", keep_on_top=True, icon=iconfile)
